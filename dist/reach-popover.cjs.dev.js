...
var tabbable__default = /*#__PURE__*/_interopDefault(tabbable);
...
function useSimulateTabNavigationForReactTree(triggerRef, popoverRef) {
  var ownerDocument$1 = ownerDocument.getOwnerDocument(triggerRef.current);
  ...
  React.useEffect(function () {
    ownerDocument$1.addEventListener("keydown", handleKeyDown);
    return function () {
      ownerDocument$1.removeEventListener("keydown", handleKeyDown);
    }; // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  function getElementAfterTrigger() {
    var elements = tabbable__default['default'](ownerDocument$1);
  ...
  function tabbedFromTriggerToPopover() {
    return triggerRef.current ? triggerRef.current === ownerDocument$1.activeElement : false;
  }
  ...
  function tabbedOutOfPopover() {
    var inPopover = popoverRef.current ? popoverRef.current.contains(ownerDocument$1.activeElement || null) : false;

    if (inPopover) {
      var elements = popoverRef.current && tabbable__default['default'](popoverRef.current);
      return Boolean(elements && elements[elements.length - 1] === ownerDocument$1.activeElement);
    }
  ...
  function tabbedToBrowserChrome(event) {
    var elements = popoverRef.current ? tabbable__default['default'](ownerDocument$1).filter(function (element) {
  ...
  function shiftTabbedToBrowserChrome(event) {
    ...
    return event.target === tabbable__default['default'](ownerDocument$1)[0];
  }
  ...
  function disableTabbablesInPopover() {
    ...
    if (elements) {
      ...
      ownerDocument$1.addEventListener("focusin", enableTabbablesInPopover);
    }
  }
